/*
    Can skip. It is easy, but the code is long.

    unordered_map[userId -> vector<{timestamp, tweetId}>]
    struct tweet {userId, timestamp, tweetId}
    priority_queue<tweet, vector<tweet>, greater_comp> pq;
        greater_comp --> tweet1.timestamp > tweet2.timestamp;
    Then it is like pick top K elements from N lists.
*/
class Twitter {
    int count;
