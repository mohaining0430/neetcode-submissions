...
public:
    ...
    void dfs(vector<int>& nums, int index, int cur_sum, int target, 
        ...
        // 1. adding nums[index], then next dfs starts with index (because nums[index] can be added multi times)
        // 2. not adding nums[index], then next dfs starts with index + 1.
